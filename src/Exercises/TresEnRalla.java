...
public class TresEnRalla {
  ...
  static void imprimirTablero(char[][] tablero) {
      ...
      for (x = 0; x < COLUMNAS; x++) {
          System.out.printf("\u001B[32m|%d", x + 1);
      }
      System.out.print("\u001B[32m|\n");
      for (y = 0; y < FILAS; y++) {
          System.out.printf("\u001B[32m|%d", y + 1);
          for (x = 0; x < COLUMNAS; x++) {
              System.out.printf("\u001B[32m|%c", tablero[y][x]);
          }
          System.out.print("\u001B[32m|\n\u001B[0m");
      }
  }
  ...
  static void colocarPieza(int y, int x, char pieza, char[][] tablero) {
      if (y < 0 || y >= FILAS) {
          System.out.print("\u001B[31m Fila incorrecta \u001B[0m");
          return;
      }

      if (x < 0 || x >= COLUMNAS) {
          System.out.print("\u001B[31m Columna incorrecta \u001B[0m");
          return;
      }
      if (pieza != JUGADOR_O && pieza != JUGADOR_X) {
          System.out.printf("\u001B[31m La pieza debe ser %c o %c \u001B[0m", JUGADOR_O, JUGADOR_X);
          return;
      }
      if (!coordenadasVacias(y, x, tablero)) {
          System.out.print("\u001B[31m Coordenadas ya ocupadas \u001B[0m");
          return;
      }
      tablero[y][x] = pieza;
  }
...
*/
  ...
  */
  ...
  static int[] elegirCoordenadasCpu(char jugador, char[][] tablero) {
      hablar("\u001B[35m Estoy pensando... \u001B[0m", jugador);
  /*
  ...
  */
      ...
      y = coordenadas[1];
      if (y != -1 && x != -1) {
          hablar("\u001B[35m Tomar victoria de oponente \u001B[0m", jugador);
          return new int[]{x, y};
      }
      // 3
      ...
      if (conteoOponente > conteoJugador) {
          hablar("\u001B[35m Tomar puntaje mayor del oponente \u001B[0m", jugador);
          return new int[]{xOponente, yOponente};
      } else if (conteoJugador > 0) {
          hablar("\u001B[35m Tomar mi mayor puntaje \u001B[0m", jugador);
          return new int[]{x, y};
      }
      // 4
      if (coordenadasVacias(0, 0, tablero)) {
          hablar("\u001B[35m Tomar columna superior izquierda \u001B[0m", jugador);
          return new int[]{0, 0};
      }
      // 5
      hablar("\u001B[35m Coordenadas aleatorias \u001B[0m", jugador);
  ...
  static void iniciarJuego(int modo) {
      if (modo != JUGADOR_JUGADOR && modo != JUGADOR_CPU && modo != CPU_CPU) {
          System.out.print("\033[31m Saliendo del juego \033[0m");
          System.exit(0);
          return;
      }
      ...
      System.out.printf("\u001B[32m El jugador que inicia es: %c\n \u001B[0m", jugadorActual);
      int x = 0, y = 0;
      // Y allá vamos
      int[] coordenadas = new int[2];
      while (true) {
          ...
          if (modo == JUGADOR_JUGADOR || (modo == JUGADOR_CPU && jugadorActual == JUGADOR_X)) {
              System.out.printf("\u001B[32m Jugador %c. Ingresa coordenadas (x,y) para colocar la pieza\n \u001B[0m", jugadorActual);
              do {
                  x = solicitarNumeroValido("\u001B[32m Ingresa X: \u001B[34m", 1, COLUMNAS);
                  y = solicitarNumeroValido("\u001B[32m Ingresa Y: \u001B[34m", 1, FILAS);
                  if (!coordenadasVacias(y - 1, x - 1, tablero)) {
                      System.out.println("\u001B[31m Coordenadas ocupadas. Intenta de nuevo \u001B[0m");
                  }
          ...
          if (comprobarSiGana(jugadorActual, tablero)) {
              imprimirTablero(tablero);
              System.out.printf("\u001B[37m El jugador %c gana\n \u001B[0m", jugadorActual);
              return;
          } else if (empate(tablero)) {
              imprimirTablero(tablero);
              System.out.print("\u001B[33m Empate \u001B[0m");
              return;
          }
      ...
      while (true) {
          ...
          if (sc.hasNextInt()) {
              ...
              if (numero >= minimo && numero <= maximo) {
                  ...
                  System.out.println("\u001B[31m Número fuera de rango. Intente de nuevo \u001B[0m");
              }
          } else {
              sc.next();
          }
  ...
  public static void Menu() {
      int modo;
      do {
    	  String menu = "\u001B[34m1. Humano contra humano\n2. Humano contra CPU (El CPU juega como " + JUGADOR_CPU_O + ")\n3. CPU contra CPU\n4. Salir\n\u001B[32m Elige: \u001B[36m";
	      modo = solicitarNumeroValido(menu, 1, 4);
	      iniciarJuego(modo);
